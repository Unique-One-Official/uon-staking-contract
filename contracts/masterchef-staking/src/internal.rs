...
impl Contract {
    ...
    pub(crate) fn claim_amount(&self, farm_id: u16, account_id: AccountId, token_type: u8) -> U128 {
        ...
        if stake_info.token_amount != U128(0) && token_type == 0 {
            if farm_info.total_token_amount == U128(0) {
                ...
                claim_amount = U128::from(
                    u128::from(stake_info.reward_token_to_claim)
                        .checked_add(
                            u128::from(stake_info.token_amount)
                                .checked_div(10000000000)
                                .unwrap()
                                .checked_mul(u128::from(farm_info.token_reward_rate))
                                .unwrap()
                                .checked_div(u128::from(farm_info.total_token_amount))
                                .unwrap()
                                .checked_mul(u128::from((now - stake_info.claimed_token_at) / 1000))
                                .unwrap()
        ...
        if stake_info.lp_share_amount != U128(0) && token_type == 1 {
            if farm_info.total_token_amount == U128(0) {
                ...
                claim_amount = U128::from(
                    u128::from(stake_info.reward_lp_to_claim)
                        .checked_add(
                            u128::from(stake_info.lp_share_amount)
                                .checked_div(1000000000000)
                                .unwrap()
                                .checked_mul(u128::from(farm_info.pool_reward_rate))
                                .unwrap()
                                .checked_div(u128::from(farm_info.total_lp_share_amount))
                                .unwrap()
                                .checked_mul(u128::from((now - stake_info.claimed_lp_at) / 1000))
                                .unwrap()
    ...
    pub(crate) fn update_claim_amounts(&mut self, farm_id: u16, token_type: u8) {
        ...
        for key in keys_vec.iter() {
            ...
            if stake_info.token_amount != U128(0) && token_type == 0 {
                stake_info.reward_token_to_claim = U128::from(
                    u128::from(stake_info.reward_token_to_claim)
                        .checked_add(
                            u128::from(stake_info.token_amount)
                                .checked_div(10000000000)
                                ...
                                .checked_mul(u128::from(
                                    (now_time - stake_info.claimed_token_at) / 1000,
                                ))
                                .unwrap()
                                .checked_mul(10000000000)
                                .unwrap(),
            ...
            if stake_info.lp_share_amount != U128(0) && token_type == 1 {
                stake_info.reward_lp_to_claim = U128::from(
                    u128::from(stake_info.reward_lp_to_claim)
                        .checked_add(
                            u128::from(stake_info.lp_share_amount)
                                .checked_div(1000000000000)
                                ...
                                .unwrap()
                                .checked_mul(u128::from(
                                    (now_time - stake_info.claimed_lp_at) / 1000,
                                ))
                                .unwrap()
                                .checked_mul(1000000000000)
                                .unwrap(),
